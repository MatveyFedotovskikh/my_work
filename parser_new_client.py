...
class Selenium_test():
    ...
    def start(self,last_date_day_month, now_date_day_month, arhiv):
        self.driver.get('https://rtschool.s20.online/company/1/customer/index?page=6&pageSize=500&sort=-first_payment_date')
        time.sleep(1)
    ...
    def client_lesson(self,name_columns = ['ФИО'], last_date_day_month = '01.01', now_date_day_month = '01.01', arhiv = False):
        ...
        #print(columns)
        all_coount = int(self.driver.find_element(By.XPATH,'//*[@id="w1"]/div[1]/span/b[2]').text)
        ...
        if count%500==0:
            ...
            count+=1
        print(1)
        for number_list_client in range(6,count+2):
            time.sleep(10)
            ...
            for number_student in range(len(students)):
                ...
                # if int(student[date_k_index].text.split('.')[0])<date_day_last and int(student[date_k_index].text.split('.')[1])<date_month_last:
                #     with open(f'info_clients\json\{now_date_day_month}_info_client.json', 'w', encoding='utf-8') as f:
            ...
            if number_list_client != count:
                self.driver.get(f'https://rtschool.s20.online/company/1/customer/index?page={number_list_client}&pageSize=500&sort=-first_payment_date')

            else:
                with open(f'info_clients\json\{now_date_day_month}_info_client_dop.json', 'w', encoding='utf-8') as f:
                    json.dump(student_id_and_name, f, ensure_ascii=False, indent=4)
                print('stop')
        with open(f'info_clients\json\{now_date_day_month}_info_client_dop.json', 'w', encoding='utf-8') as f:
            json.dump(student_id_and_name, f, ensure_ascii=False, indent=4)
